...
@app.get("/")
def hello():
    return {'message':'hello welcome to API world'}

@app.get("/about")
def info():
    return "practical learning of building an api"
...
def view_patient(patient_id : str= Path(..., description='ID of patient in the database',examples='P001')):
    data=load_data()
...
@app.get("/sort")
def sort_patients(sort_by : str = Query(..., description="sort on basis height,weight,bmi "),order : str = Query("asc",description="sort in asc or dsc")):
